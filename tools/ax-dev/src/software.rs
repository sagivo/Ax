//! Development-only software-use-case wall-clock benches: Ax / C / Rust / Go.
//!
...
//!
//! `ax-dev bench software` writes `docs/software_usecases.md`.
...
fn render_doc(rows: &[CaseResult]) -> String {
    let mut md = String::from(
        "# Software use-case benchmarks\n\n\
         ...
         as `ax-dev bench metrics`: **same algorithm**, **identical printed value**, \
         **fastest of 11 runs** (process spawn included). Ratios below 1.00× mean \
         Ax was faster. A dash means that toolchain was absent.\n\n\
         This file is regenerated by `ax-dev bench software`. The findings log at the \
    ...
    md.push_str(
        "## How to reproduce\n\n\
         ```\n\
         cargo run -p ax-dev -- bench software\n\
         ```\n\n\
...
Stood up `ax-dev bench software` and this document. Nine use cases, four languages,
identical-output gate. Existing `ax-dev bench metrics` already covers loop-carried
...
Reproduce: `ax-dev bench software`.
